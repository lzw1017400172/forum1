...
public class LoginFilter extends AbstractFilter {
    ...
    List<String> adminUrlList = null;
    @Override
    public void init(FilterConfig filterConfig) throws ServletException {
        String url = filterConfig.getInitParameter("url");
        urlList = Arrays.asList(url.split(","));//把数组转成一个集合,java自带
        String adminUrl = filterConfig.getInitParameter("adminUrl");
        adminUrlList = Lists.newArrayList(adminUrl.split(","));//google.common,把数组转成集合
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        //因为要区分管理员和用户的登录页面。。所以应该分开
        if(url.startsWith("/admin")){//以/admin开头的都是管理员页面
            if (adminUrlList != null && adminUrlList.contains(url)) {
                if (req.getSession().getAttribute("curr_admin") == null) {
                    ...
                    resp.sendRedirect("/admin/login?redirect=" + url);//jsp截取的是redirect的值，全部所以截取到的值是地址加参数
                } else {
